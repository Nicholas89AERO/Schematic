...
function LRUGhost({ x, y }: { x: number; y: number }) {
  return (
    <g opacity={ALPHA}>
      <rect x={x - 60} y={y - 30} width={120} height={60} rx={4}
        fill="#1c2a42" stroke="#f59e0b" strokeWidth={1.5} strokeDasharray="5,3" />
      <text x={x} y={y - 6} textAnchor="middle" fill="#f59e0b" fontSize={10} fontFamily="JetBrains Mono,monospace" fontWeight="600">LRU</text>
      <text x={x} y={y + 8} textAnchor="middle" fill="#94a3b8" fontSize={9} fontFamily="Inter,sans-serif">New Block</text>
    </g>
  );
}

function ExternalIfaceGhost({ x, y }: { x: number; y: number }) {
  return (
    <g opacity={ALPHA}>
...
function CircuitBreakerGhost({ x, y }: { x: number; y: number }) {
  return (
    <g opacity={ALPHA}>
      <rect x={x - 10} y={y - 10} width={20} height={20} rx={2}
        fill="#162032" stroke="#7dd3fc" strokeWidth={1.2} strokeDasharray="4,2" />
...
function RelayCoilGhost({ x, y }: { x: number; y: number }) {
  return (
    <g opacity={ALPHA}>
      <circle cx={x} cy={y} r={12} fill="#162032" stroke="#7dd3fc" strokeWidth={1.2} strokeDasharray="4,2" />
...
function FuseGhost({ x, y }: { x: number; y: number }) {
  return (
    <g opacity={ALPHA}>
...
function ConnectorGhost({ x, y }: { x: number; y: number }) {
  const pins = [0, 1, 2, 3];
  const h = pins.length * 10 + 10;
  return (
    <g opacity={ALPHA}>
      <rect x={x - 25} y={y - h / 2} width={50} height={h} rx={2}
        fill="#162032" stroke="#6366f1" strokeWidth={1.2} strokeDasharray="4,2" />
      {pins.map(i => (
...
function TerminalBlockGhost({ x, y }: { x: number; y: number }) {
  return (
    <g opacity={ALPHA}>
...
function CursorLabel({ x, y, label }: { x: number; y: number; label: string }) {
  return (
    <g>
      <rect x={x + 14} y={y - 8} width={label.length * 6 + 8} height={14} rx={3}
        fill="#1e293b" stroke="#334155" strokeWidth={0.5} fillOpacity={0.9} />
      <text x={x + 18} y={y + 1} fill="#cbd5e1" fontSize={8} fontFamily="JetBrains Mono,monospace">{label}</text>
    </g>
  );
}
...
export default function ToolGhost({ x, y, pendingStart, toolType }: GhostProps) {
  return (
    <g style={{ pointerEvents: 'none' }}>
      {toolType === 'lru_block'       && <LRUGhost x={x} y={y} />}
      {toolType === 'external_iface'  && <ExternalIfaceGhost x={x} y={y} />}
      {toolType === 'power_bus'       && <PowerBusGhost x={x} y={y} pendingStart={pendingStart} />}
      {toolType === 'circuit_breaker' && <CircuitBreakerGhost x={x} y={y} />}
      {toolType === 'relay_coil'      && <RelayCoilGhost x={x} y={y} />}
      {toolType === 'ground'          && <GroundGhost x={x} y={y} />}
      {toolType === 'fuse'            && <FuseGhost x={x} y={y} />}
      {toolType === 'connector'       && <ConnectorGhost x={x} y={y} />}
      {toolType === 'terminal_block'  && <TerminalBlockGhost x={x} y={y} />}
      {toolType === 'junction'        && <JunctionGhost x={x} y={y} />}
      {toolType === 'wire'            && <WireGhost x={x} y={y} pendingStart={pendingStart} />}
      <CursorLabel x={x} y={y} label={toolType.replace(/_/g, ' ')} />
    </g>
  );
}
